...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name) -> None:
        ...
        self.use_moe_lora = config[adapter_name].use_moe_lora
        
        if self.use_moe_lora:
            from .moe import SparseMoE
            
            assert config[adapter_name].width_choice is not None
            self.width_choice = eval(config[adapter_name].width_choice)
            
            self.lora_sparsemoe = SparseMoE(
                n_embed=self.model.config.num_hidden_layers + len(self.width_choice),
                num_experts=config[adapter_name].num_experts,
                top_k=config[adapter_name].top_k,
            )        
    ...
    def set_active_layers(self, active_layers_attn, active_layers_mlp, width=None):  # [num_layer] with boolean values
        ...
        if hasattr(self, 'use_moe_lora') and self.use_moe_lora:
            assert width is not None
            idx = self.width_choice.index(width)
            width_feature = torch.zeros(len(self.width_choice)).to(active_layers_mlp)
            width_feature[idx] = 1
            
            moe_feature = torch.cat([active_layers_mlp, width_feature])
            gating_scores, hard_indices = self.lora_sparsemoe(moe_feature)  ## assume active_layers_mlp == active_layers_attn
                
            for module in self.model.modules():
                ### Deprecated: This is for per-layer gating function
                # if hasattr(module, 'set_moe_feature'): 
                #     module.set_moe_feature(active_layers_mlp)  ## assume active_layers_mlp == active_layers_attn
                
                if hasattr(module, 'set_moe_gate'):
                    module.set_moe_gate(gating_scores, hard_indices)  ## assume active_layers_mlp == active_layers_attn
    ...
    def _create_and_replace(
        ...
        kwargs = {
            ...
            "use_moe_lora": lora_config.use_moe_lora,
            "n_embed": self.model.config.num_hidden_layers,
