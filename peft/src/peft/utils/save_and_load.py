...
def get_peft_model_state_dict(
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        ...
        to_return = {k: v for k, v in to_return.items() if (("lora_" in k and (adapter_name in k or 'lora_sparsemoe' in k)) or ("bias" in k))}
...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
    ...
    if config.peft_type in (
        ...
        for k, v in state_dict.items():
            if parameter_prefix in k and 'lora_sparsemoe' not in k:
                # print('orig:', k)
                ...
                if "." in suffix:
                    ...
                    k = f"{k}.{adapter_name}"
                # print('after process:', k)
                peft_model_state_dict[k] = v
            else:
                # print('prefix not in k:', k)
                peft_model_state_dict[k] = v
        if config.peft_type == PeftType.ADALORA:
            rank_pattern = config.rank_pattern
            if rank_pattern is not None:
