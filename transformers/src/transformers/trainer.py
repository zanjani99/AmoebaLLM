...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        if args.gradient_checkpointing:
            if args.gradient_checkpointing_kwargs is None:
                gradient_checkpointing_kwargs = {"use_reentrant": False}  ## this for solve the conflicts of gradient checkpointing + ddp + unused params
            else:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.enable_shrinking:
            if self.args.shrinking_method == 'calib_dp':
                if not hasattr(self, 'strategy'):
                    self.strategy = np.load(self.args.shrinking_file, allow_pickle=True).item()["strategy"]
                
                active_layers_list = []
                
                if self.args.layer_pruning:
                    if self.model.config.num_hidden_layers == self.args.min_num_layer:
                        active_layers_list.append(np.ones(self.model.config.num_hidden_layers))
                    else:
                        active_layers_list.append(self.strategy[self.model.config.num_hidden_layers - self.args.min_num_layer])
                else:
                    if self.model.config.num_hidden_layers == self.args.min_num_layer:
                        for _ in range(2 + self.args.random_sample_num_layer):
                            active_layers_list.append(np.ones(self.model.config.num_hidden_layers))
                    
                    else:
                        active_layers_list.append(np.ones(self.model.config.num_hidden_layers))
                        
                        num_layer_list = np.random.randint(self.args.min_num_layer, self.model.config.num_hidden_layers, self.args.random_sample_num_layer)
                        for num_layer in num_layer_list:
                            active_layers_list.append(self.strategy[self.model.config.num_hidden_layers - num_layer])  # the key of self.strategy is the num of removed layers
                        
                        active_layers_list.append(self.strategy[self.model.config.num_hidden_layers - self.args.min_num_layer])   # the key of self.strategy is the num of removed layers
                        
                active_layers_attn_list = active_layers_mlp_list = active_layers_list

            else:
                print('Not implemented:', self.args.shrinking_method)
                sys.exit()
            
            if self.args.shrinkable_width:
                width_list = []
                width_choice = eval(self.args.width_choice)
                if self.args.layer_pruning:  ## reuse the functionality of layer_pruning for width pruning
                    assert len(width_choice) == 1
                    width_list = width_choice
                else:
                    width_list.append(width_choice[0])
                    width_list.extend(np.random.choice(width_choice, self.args.random_sample_num_layer, replace=False))
                    width_list.append(width_choice[-1])
                
            # if self.args.shrinkable_width:
            #     model.model.set_active_widths(active_widths_attn, active_widths_mlp)

            for i, (active_layers_attn, active_layers_mlp) in enumerate(zip(active_layers_attn_list, active_layers_mlp_list)):                
                if self.args.shrinkable_width:
                    if self.args.nonuniform_width and i > 0 and i < len(active_layers_attn)-1:
                        pre_layer_id = 0
                        random_width = np.random.choice(width_choice)
                        for name, module in self.model.named_modules():
                            if hasattr(module, 'set_width_ratio'):
                                layer_id = int(re.search(r'layers\.(\d+)', name)[0].removeprefix('layers.'))
                                if layer_id != pre_layer_id:
                                    pre_layer_id = layer_id
                                    random_width = np.random.choice(width_choice)
                                module.set_width_ratio(random_width)
                                
                    else:
                        for module in self.model.modules():
                            if hasattr(module, 'set_width_ratio'):
                                module.set_width_ratio(width_list[i])
                    
                    self.model.set_active_layers(active_layers_attn, active_layers_mlp, width=width_list[i])
                    
                else:
                    self.model.set_active_layers(active_layers_attn, active_layers_mlp)
                
                loss_total = 0
                with self.compute_loss_context_manager():
                    if i == 0:
                        if self.args.layer_pruning and self.args.kd_weight > 0:
                            active_all = np.ones(self.model.config.num_hidden_layers)
                            
                            self.model.disable_adapter_layers()
                            self.model.set_active_layers(active_all, active_all)
                            if self.args.shrinkable_width:
                                for module in self.model.modules():
                                    if hasattr(module, 'set_width_ratio'):
                                        module.set_width_ratio(1)
                            ...
                            loss = self.compute_loss(model, inputs, teacher_logits=teacher_logits, distill_method=self.args.distill_method)
                        ...
                        if self.args.distill_method == 'gkd':
                            assert self.args.kd_weight == 0
                            model.eval()
                            with torch.no_grad():
                                new_inputs = self.generate_new_input(model, source_ids, inputs['labels'].shape[1] - source_ids.shape[1])
                                
                                self.model.set_active_layers(active_layers_attn_list[0], active_layers_mlp_list[0])
                                outputs = model(**new_inputs)
                                teacher_logits = outputs["logits"].detach()
                            model.train()
                            
                            mask = torch.ones_like(new_inputs['input_ids'])
                            mask[:, :source_ids.shape[1]] = 0

                            self.model.set_active_layers(active_layers_attn, active_layers_mlp)
                            loss = self.compute_loss(model, new_inputs, teacher_logits=teacher_logits, distill_mask=mask)
                ...
                if not self.args.no_balancing:
                    if i == 0:
                        loss_ref = loss.detach()
                    else:
                        loss = loss * loss_ref / loss.detach()
    ...
    def generate_new_input(self, model, source_ids, length):
        attention_mask = source_ids.ne(self.tokenizer.pad_token_id)
        generated_ids = model.generate(source_ids, attention_mask=attention_mask, max_new_tokens=length) # , temperature=0.7)
    ...
    def sandwich_sampling(self, up_bound, low_bound, random_sample_num, num_remain_layers=0, minimal_only=False, inter_choices=None):
        sampled_active_layers = []

        min_layer = np.pad(np.ones(low_bound), (0, up_bound-low_bound))
        if num_remain_layers > 0:
            min_layer[-num_remain_layers:] = 1

        if minimal_only:
            return [min_layer]
    
        full_layers = np.ones(up_bound)
        sampled_active_layers.append(full_layers)
        
        if inter_choices is not None and len(inter_choices) > 0:
            delta_layer_nums = [i - low_bound for i in inter_choices]
        else:
            delta_layer_nums = np.random.randint(up_bound - low_bound, size=random_sample_num)
        delta_layer_nums.sort()
        delta_layer_nums = delta_layer_nums[::-1]
        
        for delta in delta_layer_nums:
            new_active_layer = np.pad(np.ones(low_bound+delta), (0, up_bound-low_bound-delta))
            if num_remain_layers > 0:
                new_active_layer[-num_remain_layers:] = 1
            sampled_active_layers.append(new_active_layer)
        
        sampled_active_layers.append(min_layer)
        
        return sampled_active_layers
    
        
    def compute_loss(self, model, inputs, teacher_logits=None, return_outputs=False, distill_mask=None, distill_method='sp'):  ## this is the place to add runtime model control
        """
        ...
        #     print('[Debug only] Wrong code: Remember to remove!!!')
        #     loss = 0
        ...
        if teacher_logits is not None:
            if not hasattr(self, 'kd_loss_fct'):
                if distill_method == 'sp':
                    self.kd_loss_fct = KLLossSoft()
                elif distill_method == 'atkd':
                    self.kd_loss_fct = ATKDLoss()
                else:
                    print('Not implement:', distill_method)
                    sys.exit()
            if distill_mask is not None:
                mask = distill_mask
            else:
                mask = inputs['labels'] != -100 if not self.args.distill_all_tokens else None  ## -100 is the IGNORE_INDEX in qlora 

            if distill_method == 'sp':
                loss_kd = self.kd_loss_fct(outputs["logits"], teacher_logits.detach(), mask=mask)
            elif distill_method == 'atkd':
                loss_kd = self.kd_loss_fct(outputs["logits"], teacher_logits.detach(), unc_thres=self.args.unc_thres, t=inputs['labels'], mask=mask)
            
            # print(loss, loss_kd)
            # input()
            
            if self.args.distill_steps > 0 and self.state.global_step > self.args.distill_steps:
                kd_weight = 0
            else:
                kd_weight = self.args.kd_weight
            
            loss = kd_weight * loss_kd + (1 - kd_weight) * loss
            outputs["kd_loss"] = loss_kd.item()            
    ...
    def create_accelerator_and_postprocess(self):
        ...
        if self.args.ddp_find_unused_parameters:
            ...
            args["kwargs_handlers"] = [ddp_kwargs]
...
class KLLossSoft(torch.nn.modules.loss._Loss):
    ...
    """
    def forward(self, output, soft_logits, temperature=1., mask=None):
...
class ATKDLoss(torch.nn.modules.loss._Loss):
    """  
    adaptive teaching
    from https://arxiv.org/pdf/2402.11890.pdf
    """

    def KL_divergence(self, p, q, return_sum=True):
        # Make sure the probabilities do not have zero values for numerical stability
        epsilon = 1e-9
        p = p + epsilon
        q = q + epsilon
        if return_sum:
    ...
    def forward(self, p_logits, q_logits, t, unc_thres=0.5, mask=None):
        # Compute softmax probabilities for teacher and student
        p = F.softmax(p_logits, dim=-1)
        q = F.softmax(q_logits, dim=-1)

        # Handling indices for batch and time dimensions
        batch_size, time_steps = t.shape[0], t.shape[1]
        idx_batch = torch.arange(batch_size).view(-1, 1).expand(-1, time_steps).reshape(-1)
        idx_time = torch.arange(time_steps).repeat(batch_size)
        idx_labels = t.view(-1)

        # Compute binary probabilities for target class
        p_gt = p[idx_batch, idx_time, idx_labels].view(batch_size, time_steps)  # [batch_size, time_steps]
        q_gt = q[idx_batch, idx_time, idx_labels].view(batch_size, time_steps)  # [batch_size, time_steps]
        
        # print(p[0][0][t[0][0]], p_gt[0][0])
        # print(p[0][1][t[0][1]], p_gt[0][1])
        # print(p[0][2][t[0][2]], p_gt[0][2])
        # print(idx_batch.shape, idx_time.shape, idx_labels.shape, p_gt.shape)
        # input()

        # Compute probabilities among non-target classes
        p_tilde = p.clone()
        q_tilde = q.clone()
        p_tilde[idx_batch, idx_time, idx_labels] = 0
        q_tilde[idx_batch, idx_time, idx_labels] = 0
        p_tilde /= p_tilde.sum(dim=-1, keepdim=True)
        q_tilde /= q_tilde.sum(dim=-1, keepdim=True)

        # Calculate the KL divergence for target class
        kl_div_target = self.KL_divergence(p_gt, q_gt, return_sum=False)  # [batch_size, time_steps]

        # Calculate the KL divergence for non-target classes
        kl_div_non_target = self.KL_divergence(p_tilde, q_tilde)  # [batch_size, time_steps]

        kd_loss = torch.where(p_gt > unc_thres, kl_div_target + kl_div_non_target, kl_div_non_target)
